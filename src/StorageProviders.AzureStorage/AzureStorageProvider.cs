...
internal class AzureStorageProvider(AzureStorageSettings settings) : IStorageProvider
{
    ...
    Task IStorageProvider.SetMetadataAsync(string path, IDictionary<string, string>? metadata, CancellationToken cancellationToken) => SetMetadataAsync(path, metadata, cancellationToken);
}
